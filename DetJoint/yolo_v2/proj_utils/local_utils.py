...
def boxesIOU(bb_gt, bb_pred):
    gt_x_min_, gt_y_min_, gt_x_max_, gt_y_max_ = bb_gt
    gt_x_min_, gt_y_min_, gt_x_max_, gt_y_max_ = int(gt_x_min_), int(gt_y_min_), int(gt_x_max_), int(gt_y_max_)
    pred_x_min_, pred_y_min_, pred_x_max_, pred_y_max_ = bb_pred
    pred_x_min_, pred_y_min_, pred_x_max_, pred_y_max_ = int(pred_x_min_), int(pred_y_min_), int(pred_x_max_), int(pred_y_max_)
    iou_min_x = max(gt_x_min_, pred_x_min_)
    iou_min_y = max(gt_y_min_, pred_y_min_)
    iou_max_x = min(gt_x_max_, pred_x_max_)
    iou_max_y = min(gt_y_max_, pred_y_max_)

    iou_val = 0.0
    if iou_min_x < iou_max_x and iou_min_y < iou_max_y:
        common = (iou_max_y - iou_min_y) * (iou_max_x - iou_min_x)
        gt_area = (gt_y_max_ - gt_y_min_) * (gt_x_max_ - gt_x_min_)
        pred_area = (pred_y_max_ - pred_y_min_) * (pred_x_max_ - pred_x_min_)

        # iou_val = 1.0 * common / (gt_area + pred_area - common)
        iou_val = 2.0 * common / (gt_area + pred_area)

    return iou_val


def overlay_bbox_IoU(img, pred_boxes, gt_boxes, len=6):
    # Draw gt boxes
    for bb in gt_boxes:
        x_min_, y_min_, x_max_, y_max_ = bb
        x_min_, y_min_, x_max_, y_max_ = int(x_min_),int( y_min_), int(x_max_), int(y_max_)
    ...
    for bb in pred_boxes:
        ...
        for gt_bb in gt_boxes:
            cur_iou = boxesIOU(bb, gt_bb)
            if cur_iou > max_iou:
                max_iou = cur_iou

        # mid_x, mid_y = int((x_min_ + x_max_) / 2), int((y_min_ + y_max_) / 2)
        # text_loc = (mid_x, mid_y)
